<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="商品最高价" v-if="productDetail.specs.length !== 0">
        <el-input v-model="productDetail.highPrice" size="large" disabled />
      </el-form-item>

      <el-form-item label="商品价格" v-if="productDetail.specs.length === 0">
        <el-input v-model="productDetail.lowPrice" size="large" disabled />
      </el-form-item>
      ...
      <el-form-item label="商品封面图">
        <div>
          <div>
            ...
            <el-button type="primary" @click="selectAvatarFile"
              >点击上传头像</el-button
            >
          </div>
        </div>
      ...
      <!-- 保存商品详情按钮 -->
      <el-button @click="saveProductDetails" type="success" class="button-save"
        >保存商品详情</el-button
      >
    </el-form>
    ...
    >
      ...
      />
      <el-form-item v-if="productDetail.specs.length !== 0">
        <div
        ...
        >
          ...
          <el-input v-model="spec.value" placeholder="规格值" size="default" />
          <div>
            <el-button
      ...
      <el-form-item v-if="productDetail.options.length !== 0">
        ...
        >
          <el-input
            ...
            size="default"
          />
          <el-input v-model="option.info" placeholder="选项值" size="default" />
          <div>
            <el-button
...
export default {
  ...
  setup() {
    ...
    const loadingDetailText = ref('正在加载商品详情')
    ...
    const saveProductDetails = async () => {
      ...
      try {
        ...
        if (response.code === 20010) {
          notification.notifySuccess('商品详情更新成功', '')
          console.log('商品详情更新成功')
        } else {
          notification.notifyError('商品详情更新失败', response.message)
          console.error('更新商品详情失败')
        }
      } catch (error) {
        notification.notifyError('商品详情更新失败', error)
